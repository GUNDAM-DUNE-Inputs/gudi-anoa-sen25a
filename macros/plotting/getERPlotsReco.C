...
{   
    ...
    const char* inputFileName = "../outputs/gundamFitter_config_DUNE_Asimov_DryRun_badAngle.root";
    ...
    TH1D* h_cosZ_e_CC  = new TH1D("h_cosZ_e_CC",  "cos#theta_Z (#nu_{e} CC);cos#theta_Z;Events", 100, -1, 1);
    TH1D* h_cosZ_e_NC  = new TH1D("h_cosZ_e_NC",  "cos#theta_Z (#nu_{e} NC);cos#theta_Z;Events", 100, -1, 1);
    TH1D* h_cosZ_e  = new TH1D("h_cosZ_e",  "cos#theta_Z (#nu_{e});cos#theta_Z;Events", 100, -1, 1);
    TH1D* h_cosZ_mu_CC = new TH1D("h_cosZ_mu_CC", "cos#theta_Z (#nu_{#mu} CC);cos#theta_Z;Events", 100, -1, 1);
    TH1D* h_cosZ_mu_NC = new TH1D("h_cosZ_mu_NC", "cos#theta_Z (#nu_{#mu} NC);cos#theta_Z;Events", 100, -1, 1);
    TH1D* h_cosZ_mu = new TH1D("h_cosZ_mu", "cos#theta_Z (#nu_{#mu});cos#theta_Z;Events", 100, -1, 1);

    //--- 2D histograms (cos#theta_Z vs E) ---
    TH2D* h2_e_CC  = new TH2D("h2_e_CC",  "E vs cos#theta_Z (#nu_{e} CC);E [GeV];cos#theta_Z", 50, 0, 10, 100, -1, 1);
    TH2D* h2_e_NC  = new TH2D("h2_e_NC",  "E vs cos#theta_Z (#nu_{e} NC);E [GeV];cos#theta_Z", 50, 0, 10, 100, -1, 1);
    TH2D* h2_mu_CC = new TH2D("h2_mu_CC", "E vs cos#theta_Z (#nu_{#mu} CC);E [GeV];cos#theta_Z", 50, 0, 10, 100, -1, 1);
    TH2D* h2_mu_NC = new TH2D("h2_mu_NC", "E vs cos#theta_Z (#nu_{#mu} NC);E [GeV];cos#theta_Z", 50, 0, 10, 100, -1, 1);
    TH2D* h2_mu = new TH2D("h2_mu", "E vs cos#theta_Z (#nu_{#mu});E [GeV];cos#theta_Z", 50, 0, 10, 100, -1, 1);
    TH2D* h2_e = new TH2D("h2_e", "E vs cos#theta_Z (#nu_{e});E [GeV];cos#theta_Z", 50, 0, 10, 100, -1, 1);
    ...
    while ((keySubfolder = (TKey*)nextSubfolder())) {
        ...
        for (Long64_t i = 0; i < nEntries; ++i) {
            ...
            double E_e     = leafEnuRecoEle ->GetValue(0);
            double cz_e    = leafCosZRecoEle->GetValue(0);

            double E_mu     = leafEnuRecoMu ->GetValue(0);
            double cz_mu    = leafCosZRecoMu->GetValue(0);

            double w     = leafGenWeight->GetValue(0);

            //if (std::isnan(cz_mu)) continue; //std::cout << "cz_mu "<< cz_mu << std::endl;
            //std::cout << "cz_mu is finite" << std::endl;
            //if (std::isnan(cz_e)) continue; //std::cout << "cz_e: "<< cz_e << std::endl;

            if (isCC==1 && isNuE) {
                //if (!std::isfinite(cz_e)) continue;
                h_mcEnu_e_CC ->Fill(E_e,w);
                h_cosZ_e_CC  ->Fill(cz_e,w);
                h2_e_CC      ->Fill(E_e,cz_e,w);
            }
            else if(isCC==0 && isNuE) {
                //if (!std::isfinite(cz_e)) continue;
                h_mcEnu_e_NC ->Fill(E_e,w);
                h_cosZ_e_NC  ->Fill(cz_e,w);
                h2_e_NC      ->Fill(E_e,cz_e,w);
            }
            else if(isCC==1 && isNuMu) {
                //if (!std::isfinite(cz_mu)) continue;
                h_mcEnu_mu_CC->Fill(E_mu,w);
                h_cosZ_mu_CC ->Fill(cz_mu,w);
                h2_mu_CC     ->Fill(E_mu,cz_mu,w);
            }
            else if(isCC==0 && isNuMu) {
                //if (!std::isfinite(cz_mu)) continue;
                h_mcEnu_mu_NC->Fill(E_mu,w);
                h_cosZ_mu_NC ->Fill(cz_mu,w);
                h2_mu_NC     ->Fill(E_mu,cz_mu,w);
            }

            if (isNuE) {
                //if (!std::isfinite(cz_e)) continue;
                h_mcEnu_e ->Fill(E_e,w);
                h_cosZ_e  ->Fill(cz_e,w);
                h2_e      ->Fill(E_e,cz_e,w);

            }

            if (isNuMu) {
                //if (!std::isfinite(cz_mu)) continue;
                h_mcEnu_mu->Fill(E_mu,w);
                h_cosZ_mu ->Fill(cz_mu,w);
                h2_mu     ->Fill(E_mu,cz_mu,w);
    ...
    // open multi-page pdf
    c.Print("all_plots_reco_Osc_badAngle_fineBinning.pdf(");
    int count = 0;
    for (auto h : h1s) {
        c.cd();
        h->SetLineColor(kRed);
